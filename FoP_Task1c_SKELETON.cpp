...
//For other game levels make the time less
...
int main()
{
    ...
    do 
	{
        ...
        displayClock();								//Displays the timer on the console
        key = toupper(getKeyPress()); 				//read in  selected key: arrow or letter command
        timer();									//Decreases timer after the players first move
        ...
        if (minutes == 0 && seconds == 0)			//Pauses the game
    ...
    renderGame(grid, message,player);				//display game info, modified grid and messages
    if(!player.playerCheated || !(minutes == 0 && seconds == 0))      //Doesn't save the score if the player has cheated during the game or if time is 0:00
	    storeScore(player);
    if(wantsToQuit(key))
...
void displayClock()									//Displays the clock after the grid has been displayed
{
    cout << "\n         TIMER     \n";
    cout << "  --------------------\n";
...
void timer()
{
    if(true)										//Only set to if since we don't have automatic motion yet
    {
        Sleep(1);

		if (minutes == 0 && seconds == 0)			//Time runs out
